<template>
    <div v-show="!pageHide" class="cz-pagination" :class="{background}">
        ...
        <ul class="cz-pager">
            ...
            :class="{active:item===currentPage,'prev-more':item==='prev','next-more':item==='next'}" 
            v-if="typeof item ==='number'?true:layout.includes('jumper')"
    ...
    export default { 
        ...
        methods:{
            ...
            pageInit(){
                ...
                this.pages = this.unique(arr.sort((a,b)=>{return a-b}));
            },
            unique(arr){
                let obj = {};
                arr.map(item=>{
                ...
                let rawData = Object.keys(obj).map(item=>{return parseInt(item)});
                // or let rawData =  [...new Set(arr)];

                let pages = rawData.reduce((prev,current,index,arr)=>{
                    if(current>=1 && current<=this.totle){
                        prev.push(current);
                        if(arr[index+1] !== undefined && (arr[index+1] - current)>1){
                            if(prev.includes("prev")||this.currentPage < this.showNumberSize){
                                prev.push("next");
                            }else{
                                prev.push("prev");
                            }
        ...
        created(){
            if(this.currentPage && this.currentPage > this.totle) console.error("当前页必须小于或等于总页数");
            this.pageInit();
        },
        computed: {
            nextAbled(){
